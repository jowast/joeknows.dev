<script lang="ts" context="module">
    ...
    const HAND_ZOOM_STEP = (HAND_ZOOM_EXTENT[1] - HAND_ZOOM_EXTENT[0]) / HAND_ZOOM_STEP_COUNT;
...
<script lang="ts">
    ...
    function render() {
        ...
        if (hand) {
            ...
            nextHandPos.copy(handTargetPos)
                .add(new Vector3(HAND_ZOOM_EXTENT[0] + (handZoomLevel * HAND_ZOOM_STEP), 0, 0))
                .sub(hand.position);
        ...
        nextTarget.fromArray(
            nextTarget.toArray().map(v => damp(v, 0.2, 0.0001))
        );
    ...
    function onPointerMove({ detail: { original: event } }: CustomEvent<{ original: PointerEvent }>) {
    ...
    function onZoom({ detail: { diff } }: CustomEvent<{ diff: number, original: PointerEvent }>) {
